...
static std::string encodeBase58(const unsigned char *bytes, size_t len)
{
	// Count leading zeros
	size_t zeros = 0;
	...
	std::vector<unsigned char> b(bytes, bytes + len);
	...
	while(start < b.size()) {
		...
		for(size_t i = start; i < b.size(); i++) {
			int cur = (int)b[i] + carry * 256;
			int div = cur / 58;
			carry = cur % 58;
			if(temp.size() > 0 || div != 0) {
				temp.push_back((unsigned char)div);
			}
		}
		result.insert(result.begin(), BASE58_STRING[carry]);
		b = temp;
		start = 0;
	}
...
std::string Base58::encode(const unsigned char *bytes, size_t len)
{
	return encodeBase58(bytes, len);
}