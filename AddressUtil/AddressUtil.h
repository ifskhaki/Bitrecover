...
namespace Address {
	...
	// Convert a 32-byte private key to WIF (compressed=false for pre-2012 wallets)
	std::string privateKeyToWIF(const secp256k1::uint256 &privKey, bool compressed = false);
};
...
namespace Base58 {
	...
	// Encode arbitrary bytes to Base58
	std::string encode(const unsigned char *bytes, size_t len);
};
